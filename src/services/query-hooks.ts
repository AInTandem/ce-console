...
export const useWorkspacesQuery = (organizationId?: string) => {
  return useQuery<Workspace[]>({
    ...
    queryFn: () => api.getWorkspaces(organizationId),
    staleTime: 5 * 60 * 1000, // 5 minutes
...
export const useSandboxQuery = (id: string) => {
  return useQuery<Sandbox>({
    ...
    queryFn: () => {
      // We don't have a specific API call for single sandbox, so we'll get all sandboxes and find the one
      // For now, throw an error to indicate this needs to be implemented
      throw new Error('Single sandbox query not implemented');
    },
    staleTime: 5 * 60 * 1000, // 5 minutes
  });
};
...
export const useCreateOrganizationMutation = () => {
  ...
  return useMutation({
    mutationFn: ({ name, folderPath }: { name: string; folderPath: string }) => 
      api.createOrganization(name, folderPath),
    onSuccess: () => {
...
export const useCreateWorkspaceMutation = () => {
  ...
  return useMutation({
    mutationFn: ({ organizationId, name, folderPath }: { organizationId: string; name: string; folderPath: string }) => 
      api.createWorkspace(organizationId, name, folderPath),
...
export const useCreateProjectMutation = () => {
  ...
  return useMutation({
    mutationFn: ({ workspaceId, name, folderPath }: { workspaceId: string; name: string; folderPath: string }) => 
      api.createProject(workspaceId, name, folderPath),
...
export const useUpdateProjectMutation = () => {
  const queryClient = useQueryClient();
  return useMutation({
    mutationFn: ({ id, data }: { id: string; data: Partial<Omit<Project, 'id' | 'workspaceId' | 'createdAt' | 'updatedAt'>> }) => 
...
export const useDeleteProjectMutation = (workspaceId?: string) => {
  ...
  return useMutation({
    mutationFn: ({ id, deleteFolder }: { id: string; deleteFolder: boolean }) => 
      api.deleteProject(id, deleteFolder),
    onSuccess: () => {
      queryClient.invalidateQueries({ queryKey: PROJECT_KEYS.lists() });
      if (workspaceId) {
